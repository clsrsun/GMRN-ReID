...
from model_relation import Model
from utils import *
...
if dataset == 'sysu':
    data_path = 'C:/Users/sun/Desktop/zs/SYSU-MM01/'
...
suffix = dataset
if args.method == 'agw':
    suffix = suffix + '_p{}_b{}'.format(args.num_pos, args.batch_size, args.lr)
else:
    suffix = suffix + '_p{}_b{}'.format(args.num_pos, args.batch_size, args.lr)
...
# if args.method =='base':
#     net = (n_class, no_local= 'off', gm_pool =  'off', arch=args.arch)
# else:
#     net = embed_net(n_class, no_local= 'on', gm_pool = 'on', arch=args.arch)
net = Model(class_num=n_class)
net.to(device)
...
criterion_tri = OriTripletLoss()

criterion_id.to(device)
criterion_tri.to(device)

if args.optim == 'sgd':
    ignored_params = list(map(id, net.bn.parameters())) \
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        labels = torch.cat((label1, label2), 0).long().to(device)
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                ...
                train_loss=loss.item(), id_loss=loss_id.item(), tri_loss=loss_tri.item()))


def test(epoch, feat_dim=2048):
    ...
    with torch.no_grad():
        for batch_idx, (input, label) in enumerate(gall_loader):
            ...
            input = input.cuda()
            feat = net(input, input, training=False)
